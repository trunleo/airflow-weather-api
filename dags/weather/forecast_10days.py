...
FORECAST_TABLE       = Variable.get("FORECAST_DAILY_TABLE", default_var="public.weather_forecast_10days")
FORECAST_SCHEDULE    = Variable.get("FORECAST_DAILY_SCHEDULE", default_var="@daily")
TZ_STR               = Variable.get("FORECAST_DAILY_TZ", default_var="Asia/Bangkok")  # UTC+7
FLAG_ADHOC_FORECAST = str(Variable.get("FLAG_ADHOC_FORECAST_DAILY", default_var="true")).strip().lower() in {"1", "true", "yes", "y"}
ADHOC_FORECAST_CONDITION = [s.strip() for s in Variable.get("ADHOC_FORECAST_DAILY_CONDITION", default_var="678, 2994, 2789, 503").split(",")] if FLAG_ADHOC_FORECAST else None
...
API_URL     = Variable.get("FORECAST_DAILY_API_URL", default_var="https://data.tmd.go.th/nwpapi/v1/forecast/location/daily/at")
FIELDS      = Variable.get("FORECAST_DAILY_FIELDS", default_var="tc,rh,slp,rain,ws10m,wd10m,ws925,wd925,ws850,wd850,ws700,wd700,ws500,wd500,ws200,wd200,cloudlow,cloudmed,cloudhigh,cond")
TOTAL_DAYS = int(Variable.get("FORECAST_DAILY_TOTAL_DAYS", default_var="10"))
REQUEST_PAUSE_SEC = float(Variable.get("FORECAST_DAILY_REQUEST_PAUSE_SEC", default_var="0.2"))
MAX_RETRIES = int(Variable.get("FORECAST_DAILY_MAX_RETRIES", default_var="3"))
RETRY_BACKOFF_BASE_SEC = float(Variable.get("FORECAST_DAILY_RETRY_BACKOFF_BASE_SEC", default_var="1.5"))

# INSERT settings
ON_CONFLICT = Variable.get("FORECAST_DAILY_ON_CONFLICT", default_var="""(coordinate_id, forecast_date)
DO UPDATE SET
    ...
    """)  # e.g. "(lat, lon, forecast_date) DO NOTHING" or full DO UPDATE
PAGE_SIZE   = int(Variable.get("FORECAST_DAILY_PAGE_SIZE", default_var="2000"))
...
def fetch_forecast_once(lat: str, lon: str, date: str, duration: str) -> List[Dict[str, Any]]:
    """
    ...
    params = {"lat": str(lat), "lon": str(lon), "fields": FIELDS, "date": date, "duration": duration}
...
def fetch_window(lat: str, lon: str, start_dt: datetime, total_days: int) -> List[Dict[str, Any]]:
    """
    Fetch the weather forecast for a specific location over a time window.

    This function divides the total requested duration (e.g., 10 days) into smaller
    API-friendly segments (defined by MAX_DAYS_PER_CALL), then sequentially
    ...
    """
    rows = fetch_forecast_once(lat, lon, start_dt, total_days)
    time.sleep(REQUEST_PAUSE_SEC)
    
    return rows
    
def etl_weather_forecast_10days(run_date: str, **kwargs: dict) -> None:
    """
    ...
    logger.info("Start forecast ingest at %s (tz=%s) for next %sh", now_local.strftime("%Y-%m-%d"), TZ_STR, TOTAL_DAYS)
    ...
    for i, loc in enumerate(locs, start=1):
        ...
        try:
            ...
            rows = fetch_window(lat, lon, now_local.strftime("%Y-%m-%d"), TOTAL_DAYS)
    ...
    count = pg_hook_out.insert_forecast(
        ...
        page_size=PAGE_SIZE,
        column_name=[
            "forecast_date", "cloudhigh", "cloudlow", "cloudmed", "cond",
        ...
        column_value=""" %(forecast_date)s, %(cloudhigh)s, %(cloudlow)s, %(cloudmed)s, %(cond)s,
